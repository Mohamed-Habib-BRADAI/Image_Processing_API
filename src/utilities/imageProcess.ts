...
async function processFile(
    ...
    if (index == -1) {
        startProcess(fullPath, fullFile, width, height);
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            resolve([thumbPath, thumbFile]);
        }, 300);
        reject(new Error("Promise rejected"));
    });
}

async function startProcess(fullPath: string, fullFile: string, width: number |undefined, height: number |undefined): Promise<Buffer> {
